...
class TAB(ehrenfest.Ehrenfest):
  def load_hamiltonian(self, hamiltonian_path='./Hamiltonian.bin', n_states=None):
    """Load Hamiltonian matrix from binary file generated by lvc_interface.py"""
    import struct
    if n_states is None:
      import os
      file_size = os.path.getsize(hamiltonian_path)
      n_states = int(np.sqrt(file_size / (2 * 8)))
    
    with open(hamiltonian_path, 'rb') as f:
      real_data = f.read(n_states * n_states * 8)
      hamiltonian_real = np.array(struct.unpack('d' * (n_states * n_states), real_data))
      imag_data = f.read(n_states * n_states * 8)
      hamiltonian_imag = np.array(struct.unpack('d' * (n_states * n_states), imag_data))
    
    hamiltonian = (hamiltonian_real.reshape(n_states, n_states) + 
                   1j * hamiltonian_imag.reshape(n_states, n_states))
    return hamiltonian
  
  def load_dH_dR(self, dH_dR_path='./dH_dR.bin', n_states=None, n_atoms=None):
    """Load Hamiltonian derivatives dH/dR from binary file"""
    import struct, os
    if n_states is None or n_atoms is None:
      # Read from misc.bin (in same directory as dH_dR.bin)
      misc_path = os.path.dirname(dH_dR_path) + '/misc.bin'
      with open(misc_path, 'rb') as f:
        n_states, _, _ = struct.unpack('iii', f.read(12))
      n_atoms = 9  # Default for this system
    
    with open(dH_dR_path, 'rb') as f:
      dH_dR_data = f.read()
      dH_dR_flat = np.array(struct.unpack('d' * (len(dH_dR_data) // 8), dH_dR_data))
    
    dH_dR = dH_dR_flat.reshape(n_states, n_states, n_atoms, 3)
    return dH_dR
  ...
  def propagate(self, x_init, v_init, t_init, ReCn_init, ImCn_init=None):
    ...
    oldpop_adiabatic = None  # Track adiabatic populations between iterations
    ...
    while it < self.tc.config.MAXITERS: # go forever! :D
      ...
      ### SHARC LVC - perform the collapse in adiabatic basis
      states = gradout_mid["states"]
      ReCn, ImCn = gradout_mid["recn"], gradout_mid["imcn"]

      ###---Adiabatic Transformation for Collapse-------###########################
      # Diagonalize Hamiltonian (namd-model.py lines 326-328)
      # Files are in the TC temp directory
      tc_dir = gradout_mid["tdci_dir"]
      hamiltonian = self.load_hamiltonian(tc_dir + 'Hamiltonian.bin')
      dH_dR = self.load_dH_dR(tc_dir + 'dH_dR.bin')  # Load full Hamiltonian derivatives
      
      w, VR = np.linalg.eigh(hamiltonian)
      sort_idx = np.argsort(w)
      sw, sVR = w[sort_idx], VR[:, sort_idx]
      tsVR = np.transpose(sVR)
      
      # Transform to adiabatic basis (namd-model.py lines 336-349)
      ct = ReCn + 1j*ImCn
      norm2ct = np.sqrt(np.sum(np.abs(ct)**2))
      
      amp = np.zeros((dimH), dtype=np.complex)
      poparray = np.zeros((dimH))
      ampdir = np.zeros((dimH), dtype=np.complex)
      
      i = 0
      while i < dimH:
        amp[i] = np.dot(tsVR[i,:], ct) / norm2ct
        poparray[i] = (np.abs(amp[i])**2).real
        ampdir[i] = 1.0 if (poparray[i] == 0) else amp[i] / (poparray[i]**0.5)
        i = i + 1
      
      self.logprint(">Adiabatic populations before collapse: "+str(poparray))
      
      # Also log diabatic populations for comparison
      diabatic_pop = ((np.dot(ReCn, np.transpose(states)))**2 + (np.dot(ImCn, np.transpose(states)))**2).real
      self.logprint(">Diabatic populations (states basis): "+str(diabatic_pop))
      
      # Compute adiabatic forces (namd-model.py lines 356-366)
      aforces_adiabatic = np.zeros((dimH, len(aforces[0])))
      n_atoms = len(aforces[0]) // 3
      
      for atom_idx in range(n_atoms):
        for coord_idx in range(3):  # x, y, z
          flat_idx = atom_idx * 3 + coord_idx
          
          # Use precomputed dH/dR matrix (already has diagonal + off-diagonal)
          dH = dH_dR[:, :, atom_idx, coord_idx]
          
          # Transform to adiabatic: F_i = -<i|dH/dR|i>
          for i in range(dimH):
            aforces_adiabatic[i, flat_idx] = -np.dot(tsVR[i,:], np.dot(dH, sVR[:,i])).real
      
      # Calculate odotrho in adiabatic basis
      if oldpop_adiabatic is None:
        oldpop_adiabatic = np.zeros((dimH))
      odotrho = (poparray - oldpop_adiabatic) / deltatn
      
      ###---Collapsing in Adiabatic Basis-------###########################
      npop, track = self.gcollapse(dimH,deltatn,aforces_adiabatic,poparray,dcps,nzthresh,errortol,npthresh,pehrptol,odotrho,tolodotrho,nta,dtw,zpop,dgscale,tabdecay)
      self.logprint(">Adiabatic population after collapsing is "+str(npop))
      ...
      if (track == 0):
        ...
        ###---Restoring wavefunctions--------#### (namd-model.py lines 395-408)
        ...
        # Transform back to diabatic (namd-model.py line 406)
        i = 0
        while i < dimH:
      ...
      # Store adiabatic populations for next iteration
      oldpop_adiabatic = poparray.copy()
